...
stationtt <- left_join(stationtt,type, by = "STATCODE")
...
##############################Test homocedasticité

bartlett.test(data = envi, N ~ type)#non => faire krusckal
bartlett.test(data = envi, H0r ~ type)#non => faire krusckal
bartlett.test(data = envi, H1r ~ type)#oui mais pas tres beau apres
bartlett.test(data = envi, H2r ~ type)#oui mais pas tres beau apres

#################################kruskal_wallis pour N
library(tidyverse)
library(ggpubr)
library(rstatix)

ggboxplot(envi, x = "type", y = "N")

res.kruskal <- envi %>% kruskal_test(N ~ type)
res.kruskal

pwc <- envi%>% 
  dunn_test(N ~ type, p.adjust.method = "holm") 
...
ggboxplot(envi, x = "type", y = "N") +
  ...
  scale_y_continuous(breaks = round(seq(min(0), max(400), by = 30),1))

#################################kruskal_wallis pour H0r
library(tidyverse)
library(ggpubr)
library(rstatix)

ggboxplot(envi, x = "type", y = "H0r")

res.kruskal <- envi %>% kruskal_test(H0r ~ type)
res.kruskal

pwc <- envi%>% 
  dunn_test(H0r ~ type, p.adjust.method = "holm") 
...
theme(
   ...
   grids(linetype = "dashed")+
  scale_y_continuous(breaks = round(seq(min(0), max(20), by = 2),1))

#################################kruskal_wallis pour H1r
library(tidyverse)
library(ggpubr)
library(rstatix)

ggboxplot(envi, x = "type", y = "H1r")

res.kruskal <- envi %>% kruskal_test(H1r ~ type)
res.kruskal

pwc <- envi%>% 
  dunn_test(H1r ~ type, p.adjust.method = "holm") 
...
ggboxplot(envi, x = "type", y = "H1r") +
  ...
  grids(linetype = "dashed")+
  scale_y_continuous(breaks = round(seq(min(0), max(14), by = 2),1))

#################################kruskal_wallis pour H2r
library(tidyverse)
library(ggpubr)
library(rstatix)

ggboxplot(envi, x = "type", y = "H2r")

res.kruskal <- envi %>% kruskal_test(H2r ~ type)
res.kruskal

pwc <- envi%>% 
  dunn_test(H2r ~ type, p.adjust.method = "holm") 
pwc
